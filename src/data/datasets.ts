...
- 设计系统意识：优先提取设计稿中的 Design Tokens (Colors, Typography) 到全局配置。`
  },
  {
    ...
    description: 'Superpowers 元技能卡片 - 用一套可组合的 Skills 工作流约束 AI，从需求澄清、规划、实现、调试、验证到交付，减少“上来就写、写完就算完”的失控行为。',
    language: 'markdown',
    code: `@using-superpowers 先检查是否存在适用 skills；若有 1% 的可能适用就必须调用。按“流程类优先、实现类其次”的顺序推进，并在新功能、修 Bug、计划执行、完成声明等节点自动套用对应 skills。`,
    content: `---
name: using-superpowers
description: Superpowers 元技能。用于在任何任务开始前判断、调用并编排合适的 skills，把 AI 从“直接动手写代码”切换到“先选流程、再按纪律执行”。
---

# using-superpowers / Superpowers 技能系统总结

## 一句话定义

Superpowers 不是单个“万能神技”，而是一套给 AI 编程助手使用的工作流约束框架。它把设计、计划、实现、调试、验证、审查、收尾拆成可组合的 skills，并要求 AI 在行动前先决定“该走哪条流程”。

## 核心价值

1. **先流程，后动作**：防止 AI 一上来就直接改代码。
2. **把工程纪律做成硬约束**：把 TDD、调试、验证、审查从“建议”提升成“门槛”。
3. **降低上下文污染**：通过 plan、子 agent、review，把复杂任务拆开处理。
4. **让完成声明更可信**：不是“我觉得好了”，而是“有新鲜验证证据，所以可以说完成”。

## 核心规则

### 1% 原则

如果你认为某个 skill **哪怕只有 1% 的可能适用**，也必须先检查并调用，而不是跳过。

### 调用顺序

当多个 skills 都可能适用时，优先级是：

1. **流程类 skills 优先**：例如 \`brainstorming\`、\`systematic-debugging\`
2. **实现类 skills 其次**：例如 \`test-driven-development\`、领域实现技能

也就是先决定“怎么做”，再决定“做什么”。

### 刚性技能与柔性技能

- **刚性技能（Rigid）**：必须严格执行，不可随意简化
  - \`test-driven-development\`
  - \`systematic-debugging\`
  - \`verification-before-completion\`
- **柔性技能（Flexible）**：可以根据任务上下文调整呈现方式
  - \`brainstorming\`
  - \`writing-plans\`
...
- \`brainstorming\`：先把需求、约束、成功标准问清楚，避免直接开写。
- \`using-git-worktrees\`：给实现过程提供隔离工作区，减少污染主工作目录。
- \`writing-plans\`：把方案拆成小任务，假设执行者对仓库“零上下文”。
- \`subagent-driven-development\` / \`executing-plans\`：按任务逐步落地，实现时保留 review 检查点。
- \`test-driven-development\`：进入 RED → GREEN → REFACTOR 闭环。
- \`requesting-code-review\`：在关键节点主动请求审查，不等到最后一次性爆雷。
- \`finishing-a-development-branch\`：在验证通过后处理收尾、PR、合并或保留分支。
...
- **先定位根因**，而不是猜一个修法直接改。
- **再用测试保护修复**，避免只修表象。
- **最后给出新鲜证据**，证明问题真的被解决，而不是口头判断。

## 14 个核心 skills 速记

### 元技能

- \`using-superpowers\`：总入口，负责判断该用哪些 skills
- \`writing-skills\`：用于编写和维护新的 skills

### 需求与规划

- \`brainstorming\`：需求澄清、方案对比、设计收敛
- \`writing-plans\`：把设计转成可执行计划
- \`executing-plans\`：按书面计划逐项实施

### 实现与协作

- \`subagent-driven-development\`：按子任务分派独立 agent 执行
- \`dispatching-parallel-agents\`：对独立问题并行调查/实现
- \`using-git-worktrees\`：隔离开发环境
- \`requesting-code-review\`：请求代码审查
- \`receiving-code-review\`：严谨处理审查反馈
- \`finishing-a-development-branch\`：分支收尾与交付

### 质量保障

- \`test-driven-development\`：先测后写，保证最小回归面
- \`systematic-debugging\`：系统化调试，不靠猜
- \`verification-before-completion\`：完成前必须给验证证据

## 适用场景

- **做新功能**：适合完整走主链路
- **修线上/复杂 Bug**：优先走调试精简链路
- **已有明确计划**：直接进 \`executing-plans\`
- **多个互不相关的小任务**：可用 \`dispatching-parallel-agents\`

## 这张卡怎么用

当你想让 AI “别直接开写，而是按工程流程来”时，先贴这张卡片对应的 prompt，再给任务。例如：

\`\`\`markdown
@using-superpowers 先判断当前任务该用哪些 skills。
如果是新功能，先 brainstorming 再 writing-plans；
如果是修 Bug，先 systematic-debugging；
在声称完成前，必须经过 verification-before-completion。
\`\`\`
...
1. AI 会先选流程，不会直接跳到实现。
2. 对复杂任务会先做需求澄清和任务拆解。
3. 对 Bug 会先找根因，不会“拍脑袋修复”。
4. 对完成声明会更克制，必须给出验证依据。

## 一句话理解

Superpowers 的重点不是让 AI “更强”，而是让 AI **更稳、更守规矩、更像一个遵守工程流程的开发者**。`
  }
]
